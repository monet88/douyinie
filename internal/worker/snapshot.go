...
// ConfigKeyRequireModelSnapshot is the key under Command.Config indicating strict snapshot requirement.
const ConfigKeyRequireModelSnapshot = "require_model_snapshot"
...
func ValidateSnapshotPaths(env *ModelSnapshotEnvelope) error {
	...
	if ep := strings.TrimSpace(env.Primary.EntrypointFile); ep != "" {
		info, err := os.Stat(ep)
		if err != nil {
			return fmt.Errorf("primary entrypoint file inaccessible (%s): %w", ep, err)
		}
		if info.IsDir() {
			return fmt.Errorf("primary entrypoint path (%s) is a directory, expected regular file", ep)
		}
	}
